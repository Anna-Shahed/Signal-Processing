...
class Signal:
    ...
    def n_channels(self) -> int:
        """Number of channels, treating a one-dimensional signal as mono."""
    ...
    def duration(self) -> float:
        ...
        This is the sample span ``n_samples / sampling_rate`` rather than the
        timestamp of the final sample.
        """
    ...
    def normalize(self, mode: str = "peak", target: float = 1.0) -> Signal:
        """Return a normalized copy.

        Parameters
        ----------
        mode:
            ``"peak"`` scales by the largest absolute sample; ``"rms"`` scales
            by the RMS amplitude.
        target:
        ...
        if mode == "peak":
            ...
            raise SignalValidationError("mode must be either 'peak' or 'rms'.")
    ...
    def slice(self, start: int, stop: int) -> Signal:
        """Return a sample-index slice while preserving its absolute time."""
        if not isinstance(start, int) or not isinstance(stop, int):
            raise SignalValidationError("start and stop must be integer sample indices.")
        selected = self.samples[start:stop]
        if selected.shape[0] == 0:
            raise SignalValidationError("The requested slice is empty.")

        normalized_start = start if start >= 0 else max(0, self.n_samples + start)
        return self.copy(
            samples=selected,
            start_time=self.start_time + normalized_start / self.sampling_rate,
        )

    def resample(self, sampling_rate: float, *, max_denominator: int = 10000) -> Signal:
        """Resample using polyphase filtering to reduce aliasing."""
        new_rate = validate_sampling_rate(sampling_rate)
        if np.isclose(new_rate, self.sampling_rate):
            return self.copy()

        ratio = Fraction(new_rate / self.sampling_rate).limit_denominator(max_denominator)
    ...
    def from_dict(cls, data: dict[str, Any]) -> Signal:
        ...
        if missing:
            raise SignalValidationError(f"Missing signal fields: {sorted(missing)}.")
        return cls(**data)
    ...
    def from_json(cls, text: str) -> Signal:
        ...
        try:
            ...
            raise SignalValidationError("Invalid signal JSON.") from exc
        if not isinstance(data, dict):
            raise SignalValidationError("Signal JSON must contain an object.")
    ...
    def __getitem__(self, item: int | slice) -> float | np.ndarray | Signal:
        ...
        if isinstance(item, slice):
            ...
            if step != 1:
                ...
                return self.copy(
                    samples=selected,
                    start_time=self.time[start] if selected.shape[0] else self.start_time,
                )
            return self.slice(start, stop)
        return self.samples[item]

    def _binary_operation(self, other: Any, operation: Any, symbol: str) -> Signal:
        if isinstance(other, Signal):
            if not np.isclose(self.sampling_rate, other.sampling_rate):
                raise SignalValidationError(
                    f"Cannot apply '{symbol}' to signals with different sampling rates."
                )
            if self.samples.shape != other.samples.shape:
                raise SignalValidationError(
                    f"Cannot apply '{symbol}' to signals with different shapes."
                )
